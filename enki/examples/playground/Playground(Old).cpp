...
public:
		...
		ViewerWidget(world, parent)
	{
		...
		for (int i = 0; i < joystickCount; ++i)
		{
			...
			o->pos = Point(150,250);
			//o->speed = Vector(0,0);
			o->setCylindric(25, 4, 1000000);
			o->dryFrictionCoefficient = 100;
			o->setColor(Color(0, 0, 1));
	...
	void addDefaultsRobots(World *world)
	{
		...
		epuck->camera.init(0.01,world);
		//epuck->camera.wallsStep(0.1, world);
		//epuck->leftSpeed = 5;
		//epuck->rightSpeed = 5;
		world->addObject(epuck);
	...
	virtual void timerEvent(QTimerEvent * event)
	{
		...
		//epucks[0]->camera.wallsStep(0.1,world);

		epucks[0]->camera.finalize(0.01,world);
		for(int i = 0; i < epucks.size(); i++)
		{
			valarray<Color> image = epucks[i]->camera.image;
		...
		// Point P = epucks[0]->camera.getAbsolutePosition();
		// cout << P << "\t" << epucks[0]->pos.x << "," << epucks[0]->pos.y << endl;
