...
#include "include/Shepherding.h"

#ifdef Noise_Analysis
#include "include/ShepherdingNoise.h"
#endif
...
int noOfSheep, noOfShepherd, noOfObjects, mode;
float GoalRadius, GoalDistance, Sheep_MaxSpeed_Multiplier;
int Goalx, Goaly;
int global_iter;
int generation;
...
FitFunc fitnessfunction = [](const double *x, const int N)
{
  ...
  for(int i=0;i<No_Of_Trials;i++)
  {
    start:
    World world = World();
    Shepherding simulation(&world,mode,noOfSheep,noOfShepherd,noOfObjects,
      Goalx, Goaly,GoalRadius, GoalDistance, Sheep_MaxSpeed_Multiplier, x, N);
...
int main(int argc, char *argv[])
{
  //Read configuration file
	Config configfile;
	configfile.readFile("Main/config/Parameters.cfg");
...
//                   -2.03186,   1.59506};//Rerun Evo 27
 ...
 double x4[8] = { 6.32578186589613,  1.60511591621045,
  ...
  //Run simulation with GUI
    ...
    cout << endl << endl;
    ShepherdingGUI viewer(&world, mode, noOfSheep,noOfShepherd,noOfObjects,
      Goalx, Goaly,GoalRadius, GoalDistance, Sheep_MaxSpeed_Multiplier, x, dim);
  ...
  //Analysis
    ...
    for(int i=0;i<No_Of_Trials;i++)
    {
      ...
      World world = World();
      #ifdef Noise_Analysis
        ShepherdingNoise simulation(&world,mode,noOfSheep,noOfShepherd,noOfObjects,
            Goalx, Goaly,GoalRadius, GoalDistance, x, dim,Noise_Level,Noise_Scenario);
      #else
        Shepherding simulation(&world,mode,noOfSheep,noOfShepherd,noOfObjects,
            Goalx, Goaly,GoalRadius, GoalDistance, Sheep_MaxSpeed_Multiplier, x, dim);
      #endif
