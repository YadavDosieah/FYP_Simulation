...
class Shepherding
{
	...
	Shepherding(World *world, int mode, int noOfSheep, int noOfShepherd, int noOfObjects,
							...
							noOfObjects(noOfObjects), Goalx(Goalx), Goaly(Goaly),
							...
							Sheep_MaxSpeed_Multiplier(Sheep_MaxSpeed_Multiplier),	y(y), dim(dim)
	{
		...
		MaxSpeed = SPEED_MAX*Sheep_MaxSpeed_Multiplier;
	}
	...
	void CalculateSheepSpeed()
	{
		for(int i=0; i < int(flock.size()); i++)
		{
			double Force_Sheep_x = 0;
			double Force_Sheep_y = 0;
			for(int j=0; j < int(flock.size()); j++)
			{
				if(j != i)
				{
					...
					if(Distance < 10) //Sheep Repulsion
					{
						Force_Sheep_x = Force_Sheep_x + (Csheep/Distance_sq)*X_Comp;
						Force_Sheep_y = Force_Sheep_y + (Csheep/Distance_sq)*Y_Comp;
					}

					#if sheep_attraction
						else if(Distance > 25 && Distance < 35) //Sheep Attraction
						{
							Force_Sheep_x = Force_Sheep_x - Ksheep*Distance/35*X_Comp;
							Force_Sheep_y = Force_Sheep_y - Ksheep*Distance/35*Y_Comp;
						}
					#endif
				}
			}
			...
			#if obj_repulsion
				double Force_Obj_x = 0;
				double Force_Obj_y = 0;
				for(int j=0; j < int(objects.size()); j++)
				{
					double Distance_x = flock[i]->pos.x - objects[j]->pos.x;
					double Distance_y = flock[i]->pos.y - objects[j]->pos.y;
					double Distance_sq = pow(Distance_x,2) + pow(Distance_y,2);
					double Distance = sqrt(Distance_sq);
					double X_Comp = Distance_x/Distance;
					double Y_Comp = Distance_y/Distance;
					if(Distance < 10) //Sheep Repulsion
					{
						Force_Obj_x = Force_Obj_x + (Csheep/Distance_sq)*X_Comp;
						Force_Obj_y = Force_Obj_y + (Csheep/Distance_sq)*Y_Comp;
					}
				}
			#endif


			double Force_Shepherd_x = 0;
			double Force_Shepherd_y = 0;
			bool ShepherdDetected = false;
			for(int j=0; j < int(shepherds.size()); j++)
			{
					...
					if(Distance < 50)
					{
						...
						Force_Shepherd_x = Force_Shepherd_x + (Cshepherd/Distance_sq)*(Distance_x/Distance);
						Force_Shepherd_y = Force_Shepherd_y + (Cshepherd/Distance_sq)*(Distance_y/Distance);
					}
			}
			...
			{
				Force_x = Force_Shepherd_x;
				Force_y = Force_Shepherd_y;
			}
			else
			{
				#if obj_repulsion
					Force_x = Force_Sheep_x + Force_Obj_x;
					Force_y = Force_Sheep_y + Force_Obj_y;
				#else
					Force_x = Force_Sheep_x;
					Force_y = Force_Sheep_y;
				#endif
			}
			...
			{
				flock[i]->leftSpeed += distribution(generator);
				flock[i]->rightSpeed += distribution(generator);
			}
			else
			{
				flock[i]->leftSpeed = K1*Robot_Force_x + K2*Robot_Force_y  + distribution(generator);
				flock[i]->rightSpeed = K1*Robot_Force_x - K2*Robot_Force_y + distribution(generator);
			}

			if(fabs(flock[i]->leftSpeed/flock[i]->rightSpeed) > 1)
			{
				if(flock[i]->leftSpeed > MaxSpeed)
				{
					float trunc_ratio = MaxSpeed/flock[i]->leftSpeed;
					flock[i]->leftSpeed = MaxSpeed;
					flock[i]->rightSpeed *= trunc_ratio;
				}
				else if(flock[i]->leftSpeed < -MaxSpeed)
				{
					float trunc_ratio = -MaxSpeed/flock[i]->leftSpeed;
					flock[i]->leftSpeed = -MaxSpeed;
					flock[i]->rightSpeed *= trunc_ratio;
			...
			{
				if(flock[i]->rightSpeed > MaxSpeed)
				{
					float trunc_ratio = MaxSpeed/flock[i]->rightSpeed;
					flock[i]->rightSpeed = MaxSpeed;
					flock[i]->leftSpeed *= trunc_ratio;
				}
				else if(flock[i]->rightSpeed < -MaxSpeed)
				{
					float trunc_ratio = -MaxSpeed/flock[i]->rightSpeed;
					flock[i]->rightSpeed = -MaxSpeed;
					flock[i]->leftSpeed *= trunc_ratio;
	...
	void CalculateFitness()
	{
		...
		centroid_x = centroid_x/(noOfObjects+noOfSheep);
		centroid_y = centroid_y/(noOfObjects+noOfSheep);
		...
		for(int i = 0; i < noOfObjects; i++)
		{
			...
			fitness_val = fitness_val + (Distances_sq/(4*(noOfObjects+noOfSheep)*3.7*3.7))*TimeStep*0.1;
		}
	...
	void addCylinders(World *world, QVector<PhysicalObject*> *V)
	{
		...
		Obj->pos = Point(rand()%init_x, rand()%(Goaly-50));;
	...
	void addRobots(World *world, QVector<EPuck*> *V, bool shepherd)
	{
		if(shepherd)
		{
			...
			epuck->pos = Point(RandomFloat(3.7,init_x-3.7),RandomFloat(3.7,Goaly-50));
...
class ShepherdingGUI: public ViewerWidget, public Shepherding
{
	public:
	ShepherdingGUI(World *world, int mode, int noOfSheep, int noOfShepherd, int noOfObjects,
								int Goalx, int Goaly,float GoalRadius, float GoalDistance,
								float Sheep_MaxSpeed_Multiplier, const double *y, int dim, QWidget *parent = 0):
								ViewerWidget(world, parent), Shepherding(world,mode,noOfSheep,
								noOfShepherd,noOfObjects, Goalx,Goaly,GoalRadius,
								GoalDistance, Sheep_MaxSpeed_Multiplier, y, dim)
	{
